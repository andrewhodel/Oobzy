...
var doSend = Titanium.UI.createButton({
	title:'Send to '+win.groupName,
	width:280,
	height:40,
	top:140
});
...
doSend.addEventListener('click', function() {
	...
	xhr.timeout = 1000000;
	xhr.open("POST","http://oobzy.com/api/upload.php?userName="+localUser+"&password="+localPass+"&groupId="+win.groupId);
	xhr.send({
		media:file.blob,
	});

	// kill actInd once upload finishes and close tab
	xhr.onload = function() {
